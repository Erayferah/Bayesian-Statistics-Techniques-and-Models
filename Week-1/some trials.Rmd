...
## [1] 5.514068
```{r}
round(mean(theta), 1) # sample mean
```
## [1] 5.514068
```{r}
...
```{r}
ind = theta > 1.0 # set of indicators, TRUE if theta_i < 5
round(mean(ind),2) # automatically converts FALSE/TRUE to 0/1
```
## [1] 0.497
```{r}
...
```{r}
#qnorm(x=pnorm(100000,mean=0,sd=1), probs=0.3)
round(qnorm(0.30, mean=0, sd=1), 2)
